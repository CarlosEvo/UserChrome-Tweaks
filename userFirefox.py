...
vcount = {2: logging.DEBUG, 1: logging.INFO, None: logging.WARNING}

#Argument parser
parser = argparse.ArgumentParser(
    description='userChrome tweak manager for Firefox browser..')
...
class FilesBox(QHBoxLayout):
    ...
    def availTwkClick(self):
        if self.availableTweaks.currentItem().parent() is None:
            logging.debug("Tweak category selected: %s/",
                          self.availableTweaks.currentItem().text(0))
        else:
            logging.debug("Tweak selected: %s/%s",
                          self.availableTweaks.currentItem().parent().text(0),
                          self.availableTweaks.currentItem().text(0))
    ...
    def addTweak(self):
        if self.availableTweaks.currentItem().parent() is not None:
            ...
            self.tweaksAdded[tweakCat + tweakName] = QTreeWidgetItem(
                self.selectedTweaks)
    ...
    def removeTweak(self):
        if self.selectedTweaks.currentItem() is not None:
            ...
            logging.debug("Removing %s",
                          self.selectedTweaks.indexOfTopLevelItem(
                              self.selectedTweaks.selectedItems()[0]))
            self.selectedTweaks.takeTopLevelItem(
                self.selectedTweaks.indexOfTopLevelItem(
                    self.selectedTweaks.selectedItems()[0]))
            self.resizeColumns()
    ...
    def downTweak(self):
        tweakName = self.selectedTweaks.currentItem().text(2)
        tweakCat = self.selectedTweaks.currentItem().text(1)
        logging.debug("Moving down %s",
                      self.selectedTweaks.indexOfTopLevelItem(
                          self.selectedTweaks.selectedItems()[0]))
        selectedAdded = self.selectedTweaks.selectedItems()[0]
        indexOfSelected = self.selectedTweaks.indexOfTopLevelItem(
            selectedAdded)
        self.selectedTweaks.takeTopLevelItem(indexOfSelected)
        self.selectedTweaks.insertTopLevelItem(indexOfSelected + 1,
                                               selectedAdded)
